...
import { connectSocket, onGameUpdate, onGameState } from '../services/socket';

// ✅ Candle duration: 12 hours, clock-aligned (candle closes exactly at 12:00 AM and 12:00 PM)
const CANDLE_DURATION_SECONDS = 12 * 60 * 60; // 43200 seconds

// ✅ Calculates seconds remaining until the next 12:00 AM or 12:00 PM boundary.
// This keeps candles aligned to the clock even if the server/browser restarts mid-cycle,
// instead of just counting down a fixed 43200s from whenever the page loaded.
...
export default function Trading() {
  ...
  const unsubscribeUpdateRef = useRef(null);
  const timerIntervalRef = useRef(null);
  const isGeneratingCandle = useRef(false);

  // Fixed initial historical candles (7 candles)
  ...
  // Add candle and remove oldest (FIFO)
  const addCandleAndRemoveOldest = useCallback((newCandle) => {
    setCandles(prev => {
      const updatedCandles = [...prev, newCandle];
      return updatedCandles.length > 7 ? updatedCandles.slice(1) : updatedCandles;
    });
  ...
  const updateUserBalance = useCallback((newBalance) => {
    if (user && isMounted.current) {
      ...
      const updatedUsers = users.map(u => 
        u.uniqueId === user.uniqueId ? { ...u, balance: newBalance } : u
      );
  ...
  // ✅ FINAL LOGIC: WIN = Balance + (Bet × 2), LOSS = No change
  const checkBetResult = useCallback((newCandle) => {
    if (hasBet && savedPrediction && savedBetAmount > 0) {
      ...
      setBalance(prevBalance => {
        ...
        if (isWin) {
          // ✅ WIN: Add DOUBLE the bet amount (bet × 2)
          // Example: 950 + 100 = 1050 (Net profit ₹50)
          const winAmount = savedBetAmount * 2;
          newBalance = prevBalance + winAmount;
          setMessage(`🎉 YOU WON! +₹${savedBetAmount} profit | New Balance: ₹${newBalance}`);
        } else {
          // ❌ LOSS: Balance already deducted, no change
  ...
  // ✅ Timer - Generates a new candle exactly at 12:00 AM and 12:00 PM (12-hour candles)
  useEffect(() => {
    ...
    timerIntervalRef.current = setInterval(() => {
      if (isMounted.current && !isGeneratingCandle.current) {
        ...
        setTimer(prev => {
          ...
          if (newTimer <= 0) {
            console.log('⏰ 12-hour candle boundary reached, generating new candle...');
            ...
            // ✅ Recalculate against the real clock rather than just resetting to 43200,
            // so drift (tab throttling, sleep, etc.) never pushes candles off 12AM/12PM.
  ...
  // Format time as HH:MM:SS (was MM:SS — needs hours now since candles run up to 12 hours)
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        // ✅ Start the countdown aligned to the next real 12:00 AM / 12:00 PM boundary
        ...
        try {
          ...
          if (response?.data && mounted) {
            ...
            unsubscribeStateRef.current = onGameState((data) => {
              if (isMounted.current && data?.currentCandle) {
                if (data.candleHistory?.length > 0) {
                  setCandles(data.candleHistory);
                }
                setCurrentPrice(data.currentCandle?.price || currentPrice);
              }
            });
          }
        } catch (error) {
    ...
    return () => {
      ...
      if (timerIntervalRef.current) clearInterval(timerIntervalRef.current);
      if (unsubscribeStateRef.current) unsubscribeStateRef.current();
      if (unsubscribeUpdateRef.current) unsubscribeUpdateRef.current();
      if (socketRef.current) socketRef.current.disconnect();
    };
  }, [navigate, getInitialCandles]);

  // ✅ PLACE BET - Deduct amount immediately and save bet info
  const placeBetAPI = async () => {
    ...
    // ✅ Save bet info for result calculation
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-2xl p-6 mb-6 shadow-xl">
          <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-gray-400 text-xs">Next Candle</span>
              </div>
            </div>
          </div>
          ...
          <svg width="100%" height={chartHeight + 60} viewBox={`0 0 ${chartWidth} ${chartHeight + 60}`} className="w-full">
              ...
              return (
                <g key={i}>
                  <line
                    ...
                    strokeDasharray="4"
                  />
                  <text x={5} y={y + 3} fontSize="9" fill="#6b7280">
                    {Math.round(price)}
                  </text>
                </g>
              );
            ...
            <g>
              <rect
                x={startX + (candles.length * candleWidth)}
                y={5}
                width={candleWidth}
                height={22}
                fill="#1e3a5f"
              ...
              />
              <text
                x={startX + (candles.length * candleWidth) + candleWidth / 2}
                y={20}
                fontSize="11"
                fill="#60a5fa"
                textAnchor="middle"
              ...
              <rect
                x={startX + (candles.length * candleWidth) + 3}
                y={22}
                width={candleWidth - 6}
        ...
        <div className="bg-gray-800 rounded-2xl p-6">
          ...
          <div className="flex gap-4 mb-6">
            <button
              ...
              } ${(hasBet || loading) ? 'opacity-50 cursor-not-allowed' : ''}`}
            >
              🟢 PREDICT GREEN
            </button>
            <button
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <input
              ...
              disabled={hasBet || loading}
              placeholder="Custom"
              className="bg-gray-700 text-white px-4 py-2 rounded-lg w-32 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          {message && (
            <div className={`mb-6 p-4 rounded-xl text-center font-semibold ${
              message.includes('WON') ? 'bg-green-600/20 text-green-400' :
              message.includes('LOST') ? 'bg-red-600/20 text-red-400' :
              message.includes('CANDLE') ? 'bg-purple-600/20 text-purple-400' :
